    ...
    <div>
      ...
      <div className="grid gap-4 xl:grid-cols-2">
        {services.map((service) => {
          const usage = appointments.filter((appointment) => appointment.service_id === service.id).length;

          return (
            <Card key={service.id} className="editorial-card rounded-[1.75rem] border-[var(--border)] shadow-none">
              <CardContent className="p-6">
                <div className="flex items-start justify-between gap-4">
                  <div>
                    <p className="eyebrow text-[11px] text-[var(--muted)]">Service #{service.id}</p>
                    <h2 className="headline-display mt-2 text-3xl font-semibold">{service.name}</h2>
